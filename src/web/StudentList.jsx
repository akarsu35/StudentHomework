            ...
            >
              ...
              {isOpen[student.id] ? (
                <ul className='grid grid-cols-3 gap-6'>
                  {(studentHomework[student.id] || []).map((homework) => (
                    <li
                      className="flex flex-col justify-center items-center border border-yellow-600 rounded-md w-96 py-1"
                      key={homework.homework_id}
                    >
                      <h3 className='text-lg font-medium'>Ödev Adı: {homework.homeworks.name}</h3>
                      <div>
                        <button
                          onClick={() =>
                            handleStatusChange(
